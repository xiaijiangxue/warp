...
impl RequestedCommandView {
    ...
    pub fn new(
        ...
        let cancel_button = CompactibleActionButton::new(
            t!("requested_command.reject").to_string(),
        ...
        let accept_and_autoexecute_split_button = CompactibleSplitActionButton::new(
            t!("requested_command.run").to_string(),
            Some(KeystrokeSource::Fixed(
            ...
            (
                ...
                *shown.lock() = true;
                Some(render_autonomy_checkbox_setting_speedbump_footer(
                    t!("ai_output.always_allow_oz_read_only_commands").to_string(),
                    *checked,
            ...
            Some(AIActionStatus::Blocked) => {
                title = match &self.action_type {
                    RequestedActionViewType::Command => {
                        t!("requested_command.command_waiting_for_user")
                    }
                    RequestedActionViewType::McpTool => {
                        t!("requested_command.mcp_tool_waiting_for_user")
                    }
                };
            }
                            ...
                            .and_then(|block| block.long_running_control_state())
                        {
                            match long_running_command_control_state {
                                LongRunningCommandControlState::Agent { is_blocked, .. } => {
                                    ...
                                    if is_errored {
                                        t!("requested_command.agent_errored")
                                    } else if *is_blocked {
                                        t!("requested_command.agent_needs_input")
                                    } else {
                                        t!("requested_command.monitoring_command")
                                    }
                                }
                            ...
                            t!("requested_command.viewing_command_detail")
                        }
                    }
                    RequestedActionViewType::McpTool => {
                        t!("requested_command.viewing_mcp_tool_detail")
                    }
                };
            }
            None => {
                if self.block_model.status(app).is_streaming() {
                    title = t!("requested_command.generating_command");
                    ...
                    title = if title_str.trim().is_empty() {
                        t!("requested_command.generating_command")
                    } else {
                        title_str.into()
                    };
...
pub(crate) fn header_message_for_user_take_over_reason(
    ...
    match reason {
        UserTakeOverReason::Manual => t!("requested_command.user_in_control"),
        UserTakeOverReason::Stop => t!("requested_command.paused_user_in_control"),
        UserTakeOverReason::TransferFromAgent { .. } => {
            t!("requested_command.user_in_control_short")
        }
    }
}
