            ...
            ExpireApiKeyButtonEvent::ExpireApiKeySucceeded { uid } => {
                ...
                crate::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::success(
                        t!("platform.api_key_deleted").to_string(),
                    );
                    toast_stack.add_ephemeral_toast(toast, window_id, ctx);
                });
                ctx.notify();
            ...
            Flex::row()
                ...
                .with_child(
                    ui_builder
                        ...
                        .with_text_label(t!("platform_ext.create_api_key").to_string())
                        .build()
                        .on_click(|ctx, _, _| {
            ...
            Align::new(
                Flex::column()
                    ...
                    .with_child(
                        Container::new(
                            Text::new(
                                t!("platform.no_api_keys").to_string(),
                                appearance.ui_font_family(),
                                SUBHEADER_FONT_SIZE,
                            )
                            .with_color(appearance.theme().active_ui_text_color().into())
                    ...
                    .with_child(
                        Container::new(
                            Text::new(
                                t!("platform.create_key_description").to_string(),
                                appearance.ui_font_family(),
                                CONTENT_FONT_SIZE,
                            )
                            .with_color(appearance.theme().active_ui_text_color().into())
