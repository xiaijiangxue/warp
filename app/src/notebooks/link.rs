    ...
    pub fn secondary_action(&self) -> Option<SecondaryAction> {
        match self {
            LinkTarget::LocalDirectory { .. } => Some(SecondaryAction {
                label: t!("notebook.new_session").to_string().into(),
                tooltip: Some(
                    t!("notebook.open_new_session_in_directory")
                        .to_string()
                        .into(),
                ),
                accessibility_content: t!("notebook.open_in_terminal_session").to_string().into(),
            }),
            LinkTarget::LocalFile {
                is_markdown: true, ..
            } => Some(SecondaryAction {
                label: t!("notebook.open_in_editor").to_string().into(),
                tooltip: None,
                accessibility_content: t!("notebook.edit_markdown_file").to_string().into(),
            }),
...
impl fmt::Display for ResolveError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ResolveError::FileNotFound => write!(f, "{}", t!("notebook.file_not_found")),
            ResolveError::MissingContext => write!(f, "{}", t!("notebook.no_base_directory")),
            ResolveError::Unknown => write!(f, "{}", t!("notebook.broken_file_link")),
        }
    }
}
