    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let dialog = if self.requested_download {
            ...
            .with_bottom_row_child(Self::render_dialog_button(
                t!("wasm_nux.open_in_warp").to_string(),
            ...
            Dialog::new(
                t!("wasm_nux.download_desktop_title").to_string(),
                None,
                dialog_styles,
            )
            .with_child(
                Flex::column()
                    ...
                    .with_child(
                        appearance
                            .ui_builder()
                            .span(t!("wasm_nux.download_description").to_string())
            ...
            .with_bottom_row_child(Self::render_dialog_button(
                t!("wasm_nux.download").to_string(),
                WasmNUXDialogAction::OpenDownloadDesktopAppLink,
                &self.download_warp_mouse_state,
                appearance,
            ))
        } else {
            let title = match web_intent_parser::current_web_intent() {
                Some(WebIntent::DriveObject(_)) => {
                    t!("wasm_nux.always_open_drive_on_web").to_string()
                }
                Some(WebIntent::SessionView(_)) => {
                    t!("wasm_nux.always_open_shared_sessions_on_web").to_string()
                }
                _ => t!("wasm_nux.always_open_links_on_web").to_string(),
            };
